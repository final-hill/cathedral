...
<template>
    ...
    <UTabs :items="tabItems">
        <template #user-stories>
            <p class="whitespace-pre-wrap mb-8">{{ UserStory.description }}</p>

            <XWorkflow :organization-slug="organizationSlug" :req-type="ReqType.USER_STORY"
                :solution-slug="solutionSlug" />
        </template>
        <template #use-cases>
            <p class="m-8"> {{ UseCase.description }} </p>
