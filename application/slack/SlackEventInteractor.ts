...
import { SlackRepository, PermissionRepository, RequirementRepository, OrganizationCollectionRepository, OrganizationRepository } from "~/server/data/repositories";
import type { z } from "zod";
import { slackAppMentionSchema, slackBodySchema, slackMessageSchema, slackSlashCommandSchema, type SlackInteractivePayload, type SlackResponseMessage } from "~/server/data/slack-zod-schemas";
import { ReqType, type ParsedRequirements, PermissionDeniedException, MismatchException } from "#shared/domain";
import cache from "~/server/utils/cache";
import { SlackWorkspaceInteractor } from "./SlackWorkspaceInteractor";
import { SlackChannelInteractor } from "./SlackChannelInteractor";
import { SlackUserInteractor } from "./SlackUserInteractor";
...
export class SlackEventInteractor extends Interactor<z.infer<typeof slackBodySchema>> {
    ...
    async handleEvent(body: unknown) {
        ...
        switch (parsed.type) {
            ...
            default:
                throw new MismatchException(`Unhandled request type: ${(parsed as any).type}`);
        }
    }
    ...
    private async processRequirementsParsing(
        ...
        if (!teamId)
            return await this._slackService.sendTeamInfoError(data.channel, data.ts);

        const channelConfig = await this._channelInteractor.getChannelConfiguration(data.channel, teamId);
        if (!channelConfig)
            return await this._slackService.sendChannelNotLinkedError(data.channel, data.ts);
    ...
    private async parseRequirementsWithInteractor(params: {
        ...
        try {
            ...
            return await this._slackService.sendParsingError(channelId, thread_ts);
        }
    }
