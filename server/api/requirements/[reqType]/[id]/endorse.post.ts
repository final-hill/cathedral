...
export default defineEventHandler(async (event) => {
    const { id } = await validateEventParams({ event, schema: paramSchema }),
        ...
        { solutionSlug, organizationId: orgId, organizationSlug: orgSlug, comments } = await validateEventBody({ event, schema: bodySchema }),
    ...
    return reviewInteractor.endorseRequirement({
        requirementId: id,
        comments
