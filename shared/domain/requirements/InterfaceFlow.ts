...
export const InterfaceFlow = InterfaceArtifact.extend({
    ...
    states: z.array(z.string()).prefault([]).describe('List of states in the flow'),
    initialState: z.string().optional().describe('Starting state of the flow'),
    finalStates: z.array(z.string()).prefault([]).describe('Ending states of the flow'),
    transitions: z.string().optional().describe('State transition rules and conditions'),
    reqType: z.enum(ReqType).prefault(ReqType.INTERFACE_FLOW)
