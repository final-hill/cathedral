...
const form = useTemplateRef('form'),
    formState = reactive(Object.create(props.state))
...
const onCancel = () => {
    form.value?.clear()
    Object.assign(formState, props.state)
...
<template>
    <UForm ref="form" :id="props.id" :state="formState" :schema="props.schema"
        :class="`gap-4 flex flex-col ${props.class}`" @submit="onSubmit" autocomplete="off">
        <UFormField v-for="field of getSchemaFields(props.schema)" :label="field.label" :name="field.key"
            :key="field.key" :field="field.key" :required="!field.isOptional" :description="field.description"
            :hint="field.isReadOnly ? '(Read Only)' : undefined" size="xl" class="w-full"
            :help="field.innerType instanceof z.ZodString && field.maxLength && field.maxLength > 254 ? `Max length: ${formState[field.key]?.length ?? 0}/${field.maxLength}` : undefined">

            <UCheckbox v-if="field.innerType instanceof z.ZodBoolean" v-model="formState[field.key]" class="w-full" />
            <UInput type="text"
                v-else-if="field.innerType instanceof z.ZodString && field.maxLength && field.maxLength <= 254"
                v-model.trim="formState[field.key]" class="w-full" aria-describedby="character-count"
                :ui="{ trailing: 'pointer-events-none' }">
                <template #trailing>
                    <div id="character-count" class="text-xs text-(--ui-text-muted) tabular-nums" aria-live="polite"
                        role="status">
                        {{ formState[field.key]?.length ?? 0 }}/{{ field.maxLength }}
                    </div>
                </template>
            </UInput>
            <UTextarea v-else-if="field.innerType instanceof z.ZodString && field.maxLength && field.maxLength > 254"
                v-model.trim="formState[field.key]" class="w-full" autoresize>
            </UTextarea>
            <UInput v-else-if="field.isReadOnly" type="text" v-model.trim="formState[field.key]" disabled tabindex="-1"
                class="w-full" />
            <UInputNumber v-else-if="field.innerType instanceof z.ZodNumber" v-model.trim="formState[field.key]"
                :min="field.min" :max="field.max" class="w-full" />
            <UInput type="datetime-local" v-else-if="field.innerType instanceof z.ZodDate"
                v-model.trim="formState[field.key]" class="w-full" />
            <USelect v-else-if="field.isEnum" v-model="formState[field.key]" :items="field.enumOptions"
                class="w-full" />
            <UInputMenu v-else-if="field.isObject" v-model="formState[field.key]" :items="autocomplete || []"
                :loading="autocompleteStatus === 'pending'" class="w-full" />
            <!-- :loading="(await fetchAutocompleteData(formState[field.key])).status === 'pending'" -->
            <UInput v-else v-model.trim="formState[field.key]" class="w-full" />
        </UFormField>
