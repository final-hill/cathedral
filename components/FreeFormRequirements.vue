...
async function submitRequirements({ data: { description } }: FormSubmitEvent<RequirementsFormSchema>) {
    try {
        ...
        const parsedResult = await $fetch<z.infer<typeof ParsedRequirements>>(`/api/requirements/${ReqType.PARSED_REQUIREMENTS}/${parsedResultId}`, {
            method: 'GET',
            params: {
                solutionSlug: props.solutionSlug,
                organizationSlug: props.organizationSlug
...
<template>
    <UCard class="w-2xl m-auto">
        <template #header>
            <h2 class="text-lg font-semibold leading-6">Add Requirement</h2>
        ...
        <UAlert v-if="parsedResultCount" title="Parsing Complete" icon="i-lucide-check-circle" color="success"
            :description="`Parsed ${parsedResultCount.requirementCount} requirements`" :actions="parsedResultActions" />
        <UAlert v-if="parsedResultError" title="Parsing Failed" icon="i-lucide-alert-triangle" color="error"
            :description="parsedResultError" />
        <UForm class="flex flex-col gap-6 h-60 mt-4" @submit.prevent="submitRequirements" :state="requirementsFormState"
            :schema="requirementsFormSchema">
            <UFormField label="Statement" name="requirements" field="requirements" size="xl" required>
                <UTextarea v-model="requirementsFormState.description" class="w-full" autoresize />
            ...
            <div class="flex gap-2">
                <UButton type="submit" color="success" label="Submit" size="xl"
                    :disabled="requirementsFormState.description === '' || requirementsFormState.description.length > schemaField.maxLength!" />
                <UButton type="button" color="secondary" label="Reset" size="xl" @click="resetRequirements" />
            </div>
        </UForm>
    </UCard>
</template>